...
std::size_t list_size(const node<T>*& head{
  if(head == nullptr){
    ...
    1 + list_size(head->link());
  }
}

node<T>* list_copy(const node<T>*& head){
  if(){
...
// pre: the nodes in the given list contain the digits of a decimal
// nonnegative integer b;
// head points to the least significant (units) digit;
// 0 <= d <= 9
// post: the nodes in the given list now contain the digits of b * d
void multiply(node<int> * & head, node<int> * & tail, int d){
  ...
  for(node<int>* p = head; p!=nullptr; p = p->link()){
    product = p->data() * d + carry_over;
    p->setdata(proudct%10);
    carry = product/10;
  }
  // add last carryOver
  if(carry_over != 0)
    list_tail_insert(head, tail, carry_over);
}
...
node<T> * unique_copy(const node<T> * head){
  ...
  for(node<T> *p = head; p!=nullptr; p = p->link()){
    if(!list_search(p->link(), p->data()))
      list_head_insert(new_head, p->data());
  }
  return new_head;
}
